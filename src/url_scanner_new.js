...
client = client.replace(/_/g, ' '); // Replacing underscores with whitespace
...
function stringifyObjects(objects) {
    return objects.map(obj => `${obj.url}, ${obj.id}`).join('\n');
}
...
async function classifyURLs(urls, concurrentLimit) {
    ...
    for (const url of urls) {
        ...
        if (evaluatedCount % 20 === 0 || evaluatedCount == urlsLength + 1) {
            ...
            await updateInfringmentStatus(deadSites);
...
async function patchInfringementStatus(infringementObjects, infringmentStatus) {
    // map with status
    try {
        const requestData = {
...
async function updateInfringmentStatus(infringementObjects) {
    await patchInfringementStatus(infringementObjects, 'Removed');
}
